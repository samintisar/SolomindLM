 ...
 * for ZhipuAI glm-image generation (with text rendering) and Supabase upload.
 * 
 ...
 * and uploads generated images to Supabase storage.
 */
class SlideImageGenerationService {
  private client: ZhipuAI;
  private supabaseUrl: string;
  private supabaseKey: string;
  private bucketName = 'slide-decks';
   ...
   */
  async uploadImageToSupabase(imageBuffer: Buffer, slideNumber: number, slideDeckId: string): Promise<string> {
    const fileName = `${slideDeckId}/slide-${slideNumber}-${Date.now()}.png`;

    logInfo({
      agent: 'SlideDeckGraph',
      phase: 'upload_to_supabase',
      slideNumber,
      fileName,
      fileSize: imageBuffer.length,
    } as any, `Uploading slide ${slideNumber} to Supabase...`);
    ...
    try {
      ...
      } as any, `Failed to upload slide ${slideNumber} to Supabase`);
  ...
  async generateSlideImages(slides: Slide[], slideDeckId: string, concurrency: number = 1): Promise<Slide[]> {
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      try {
        ...
        // Upload to Supabase
        const imageUrl = await this.uploadImageToSupabase(imageBuffer, slide.slideNumber, slideDeckId);
