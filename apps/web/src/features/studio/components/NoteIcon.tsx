...
export const NoteIcon: React.FC<NoteIconProps> = ({ note, onPlayAudio }) => {
  ...
  // Type-specific icons
  const iconConfig: Record<string, { icon: React.FC<{ className?: string }>; bgClass: string; textClass: string }> = {
    ...
    writtenQuestions: { icon: MessageSquareText, bgClass: 'bg-green-500/10', textClass: 'text-green-700' },
  ...
  if (config) {
    ...
    return (
      <div className={`shrink-0 w-8 h-8 rounded-lg ${config.bgClass} ${config.textClass} flex items-center justify-center`}>
