import type { ConcreteRunnerKind, FailureCategory, FailureGroup, MetricResult } from "../types";

/**
 * Map metric names and breakdown categories to failure categories.
 */
function classifyFailure(metric: MetricResult): FailureCategory {
  if (metric.metric.startsWith("binary_judge_studio_structure")) {
    return "studio_structure";
  }
  if (metric.metric.startsWith("binary_judge_studio_grounding")) {
    return "studio_grounding";
  }
  if (
    metric.metric.startsWith("studio_") ||
    metric.metric.includes("written_questions") ||
    metric.metric.includes("flashcard") ||
    metric.metric.includes("quiz_structure") ||
    metric.metric.includes("mindmap") ||
    metric.metric.includes("infographic") ||
    metric.metric.includes("spreadsheet") ||
    metric.metric.includes("audio_script")
  ) {
    return "studio_generation";
  }

  if (metric.metric === "abstention_correctness") {
    const category = (metric.breakdown?.category as string) ?? "prompt_answering";
    if (category === "retrieval_coverage") return "retrieval";
    if (category === "context_selection") return "context_selection";
    return "prompt_answering";
  }

  if (metric.metric.startsWith("retrieval_item_recall_")) {
    return "retrieval";
  }

  if (metric.metric === "retrieval_precision_at_k" || metric.metric === "retrieval_ndcg_at_k") {
    return "context_selection";
  }

  if (metric.metric === "lr_search_yield" || metric.metric === "lr_ranking_top_relevance") {
    return "literature_review_search";
  }
  if (metric.metric === "lr_screening_inclusion_rate") {
    return "literature_review_screening";
  }
  if (
    metric.metric === "lr_extraction_coverage" ||
    metric.metric === "lr_extraction_depth" ||
    metric.metric === "lr_llm_judge_extraction_quality"
  ) {
    return "literature_review_extraction";
  }
  if (
    metric.metric === "lr_report_citation_coverage" ||
    metric.metric === "lr_llm_judge_report_quality" ||
    metric.metric === "literature_review_report_sections" ||
    metric.metric === "lr_citation_key_validity" ||
    metric.metric === "lr_required_section_names" ||
    metric.metric === "lr_summary_of_evidence_present" ||
    metric.metric === "lr_prisma_consistency" ||
    metric.metric.startsWith("binary_judge_lr")
  ) {
    return "literature_review_report";
  }
  if (metric.metric === "lr_expected_item_recall") {
    return "retrieval";
  }

  if (metric.metric.startsWith("binary_judge_chat_grounding")) {
    return "context_selection";
  }
  if (metric.metric.startsWith("binary_judge_citation")) {
    return "citation";
  }
  if (metric.metric.startsWith("binary_judge_research_grounding")) {
    return "research_planning";
  }
  if (metric.metric.startsWith("binary_judge_research_plan")) {
    return "research_planning";
  }

  if (metric.metric === "citation_validity") return "citation";
  if (metric.metric === "latency_cost_budget") return "latency_cost";
  if (metric.metric === "expected_item_recall") return "retrieval";

  return "prompt_answering";
}

/**
 * Suggested fix scope per failure category (pipeline-first per AGENTS.md).
 */
const FIX_SUGGESTIONS: Record<FailureCategory, { scope: string; files: string[] }> = {
  retrieval: {
    scope:
      "Improve vector/keyword search, hybrid configuration, or chunking so expected items appear in retrieved chunks",
    files: [
      "convex/_agents/chat/hybrid_search.ts",
      "convex/_agents/chat/vector_search.ts",
      "convex/_services/processing/",
    ],
  },
  context_selection: {
    scope:
      "Adjust context selection thresholds, token budget, or reranking to retain relevant chunks",
    files: [
      "convex/_agents/chat/chunkContext.ts",
      "convex/_agents/chat/chatConfig.ts",
      "convex/_agents/chat/hybrid_search.ts",
    ],
  },
  prompt_answering: {
    scope:
      "Check retrieval coverage and chunk selection first; only then review answer-generation if chunks already contain the facts",
    files: [
      "convex/_agents/chat/chunkContext.ts",
      "convex/_agents/chat/hybrid_search.ts",
      "convex/_agents/chat/ChatAgent.ts",
    ],
  },
  citation: {
    scope: "Fix citation extraction or grounding validation to produce accurate references",
    files: ["convex/_agents/chat/grounding_validator.ts", "convex/_agents/chat/llm_wrapper.ts"],
  },
  research_planning: {
    scope: "Improve research plan decomposition or evidence gathering for complex queries",
    files: [
      "convex/_agents/research/graph.ts",
      "convex/_agents/research/nodes.ts",
      "convex/_agents/research/prompts.ts",
    ],
  },
  latency_cost: {
    scope:
      "Reduce latency or token usage — check redundant LLM calls, oversized contexts, or missing caches",
    files: [
      "convex/_agents/chat/chatConfig.ts",
      "convex/_agents/chat/ChatAgent.ts",
      "convex/studio/scheduling/",
    ],
  },
  literature_review_search: {
    scope: "Improve academic search query generation or search API configuration",
    files: [
      "convex/_agents/literature_review/prompts.ts",
      "convex/_services/search/AcademicSearchService.ts",
      "convex/literatureReview/workflowSteps.ts",
    ],
  },
  literature_review_screening: {
    scope: "Adjust PRISMA screening prompts or batch size for inclusion accuracy",
    files: [
      "convex/_agents/literature_review/prompts.ts",
      "convex/literatureReview/workflowSteps.ts",
    ],
  },
  literature_review_extraction: {
    scope: "Improve extraction prompts or column definitions for coverage and accuracy",
    files: [
      "convex/_agents/literature_review/prompts.ts",
      "convex/literatureReview/workflowSteps.ts",
      "convex/literatureReview/db.ts",
    ],
  },
  literature_review_report: {
    scope: "Improve report structure/citation handling in the literature review workflow",
    files: [
      "convex/_agents/literature_review/prompts.ts",
      "convex/literatureReview/workflowSteps.ts",
    ],
  },
  studio_structure: {
    scope: "Fix studio output schema validation, parsing, or generation graph for the failing kind",
    files: ["convex/studio/scheduling/", "convex/_agents/", "evals/rag/metrics/studio.ts"],
  },
  studio_grounding: {
    scope:
      "Improve notebook RAG retrieval/selection for studio jobs before changing generation prompts",
    files: [
      "convex/studio/",
      "convex/_agents/chat/chunkContext.ts",
      "convex/_agents/chat/hybrid_search.ts",
    ],
  },
  studio_generation: {
    scope:
      "Inspect studio agent graph, scheduling, and reduce-phase retrieval for the failing kind",
    files: ["convex/studio/scheduling/", "convex/_agents/", "convex/studio/"],
  },
};

export interface GroupFailuresOptions {
  /** Include metrics with "warn" status in failure groups (default: true) */
  includeWarnings?: boolean;
}

/**
 * Group failed metric results into FailureGroups for coding agent consumption.
 */
export function groupFailures(
  metrics: MetricResult[],
  options?: GroupFailuresOptions
): FailureGroup[] {
  const includeWarnings = options?.includeWarnings ?? true;

  const failures = metrics.filter(
    (m) => m.status === "fail" || (includeWarnings && m.status === "warn")
  );

  if (failures.length === 0) return [];

  const groups = new Map<
    string,
    {
      category: FailureCategory;
      cases: Map<string, { caseId: string; runner: string; failures: MetricResult[] }>;
    }
  >();

  for (const metric of failures) {
    const category = classifyFailure(metric);
    const key = `${category}::${metric.runner}`;

    if (!groups.has(key)) {
      groups.set(key, { category, cases: new Map() });
    }
    const group = groups.get(key)!;

    const caseKey = `${metric.caseId}::${metric.runner}`;
    if (!group.cases.has(caseKey)) {
      group.cases.set(caseKey, {
        caseId: metric.caseId,
        runner: metric.runner,
        failures: [],
      });
    }
    group.cases.get(caseKey)!.failures.push(metric);
  }

  return Array.from(groups.values()).map(({ category, cases }) => {
    const fix = FIX_SUGGESTIONS[category];
    return {
      category,
      cases: Array.from(cases.values()).map((c) => ({
        caseId: c.caseId,
        runner: c.runner as ConcreteRunnerKind,
        failures: c.failures,
        traceHints: c.failures.map((f) => `${f.metric}: ${f.detail}`),
      })),
      suggestedFix: fix.scope,
      targetFiles: fix.files,
    };
  });
}
