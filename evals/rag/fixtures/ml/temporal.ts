...
export const mlTemporalFixtures: EvalFixture[] = [
  ...
  },
  {
    ...
    question: "How does the sequential forecasting approach predict time series values multiple steps into the future using a single model and a for loop?",
    expectedItems: [],
    expectedAnswer: "In the sequential forecasting approach, the model predicts one step ahead and then pretends that prediction is the true value for subsequent steps. For example, you first predict the target for the immediate next time step. Then, to predict the step after that, you use your first prediction as the truth for your lag features. To predict the third step, you use your predictions for both the first and second steps as inputs, and this iterative process continues sequentially to predict further into the future.",
    expectedBehavior: "Must describe the chronological progression of predicting one step, feeding that prediction back into the model as a lagged feature 'truth', and repeating the process iteratively to predict subsequent time steps.",
    notebookId: "jd72h9qsq5zap11ede5k8rqkx585djmc",
