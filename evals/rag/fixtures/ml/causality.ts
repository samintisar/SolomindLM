...
export const mlCausalityFixtures: EvalFixture[] = [
  {
    ...
    question:
      "Why is random shuffling inappropriate when splitting time series data into training and validation sets?",
    expectedItems: [],
    expectedAnswer:
    ...
    expectedBehavior:
      "Must explain that random splitting leaks future information into the training set. Must mention that deployment scenarios require predicting the future from the past, so chronological (time-based) splitting is required to prevent data leakage.",
  ...
  },
  {
    ...
    question:
      "Why does a Random Forest classifier build each tree using a random bootstrap sample and a random subset of features at each node?",
    expectedItems: [],
    expectedAnswer:
    ...
    expectedBehavior:
      "Must explain the goal of reducing overfitting. Must explicitly link the two forms of randomness (bootstrap sampling and random feature subsets) to the creation of a *diverse* set of trees whose averaged output generalizes better than a single tree.",
    notebookId: "jd72h9qsq5zap11ede5k8rqkx585djmc",
  ...
  },
  {
    ...
    question:
      "In the context of the fundamental tradeoff, why does a model with very high complexity often exhibit a large gap between training error and validation error?",
    expectedItems: [],
    expectedAnswer:
      "As model complexity increases, the training error tends to go down but the validation error tends to go up, which is known as the fundamental tradeoff. A highly complex model, such as a decision tree with unlimited maximum depth, will learn unreliable patterns and random quirks in order to get every single training example correct. This results in a very low training error, but a high validation error because the model fails to generalize to new data, a scenario known as overfitting or high variance.",
    expectedBehavior:
      "Must mention that high complexity leads to memorizing noise/quirks in the training data (overfitting or high variance). Must state that this drives training error down while validation error increases or remains high, creating the gap.",
    notebookId: "jd72h9qsq5zap11ede5k8rqkx585djmc",
  ...
  },
  {
    ...
    question:
      "Why does optimization bias occur when repeatedly using a single validation set for hyperparameter tuning?",
    expectedItems: [],
    expectedAnswer:
      "Optimization bias occurs because evaluating a large number of hyperparameter combinations increases the probability of finding a model that performs well on the validation data purely by chance. Because the validation set is over-used to search through the complexity of the model space, the chosen model ends up overfitting to the specific quirks of that validation set. Consequently, the validation error shrinks artificially and is no longer a reliable or unbiased approximation of how well the model will generalize to new, unseen data.",
    expectedBehavior:
      "Must explain that trying many hyperparameter configurations increases the statistical chance of getting a high score randomly. Must explicitly mention that this causes the model to 'overfit the validation set' and makes the validation error an overly optimistic estimate of generalization.",
    notebookId: "jd72h9qsq5zap11ede5k8rqkx585djmc",
  ...
  },
  {
    ...
    question:
      "Why are tree-based models, such as Random Forests, unable to successfully forecast a continuous upward trend in time series data?",
    expectedItems: [],
    expectedAnswer:
      "Tree-based models make predictions by learning split rules and thresholds strictly derived from the training data, such as routing data based on whether a 'days_since' feature is greater than a specific value seen in training. Because they rely entirely on the bounding regions of the training set to make splits, they are inherently unable to extrapolate to values outside the range of the training data. Therefore, for any future time steps that exceed the maximum time value observed during training, the model cannot predict continued growth and will simply output a constant prediction based on the highest training threshold.",
    expectedBehavior:
      "Must explain the mechanism of tree splits (relying on thresholds established by training data). Must explicitly conclude that this mechanism prevents the model from extrapolating beyond the maximum feature values observed in the training set.",
    notebookId: "jd72h9qsq5zap11ede5k8rqkx585djmc",
  ...
  },
  {
    ...
    question:
      "In the context of handling class imbalance, why does the Synthetic Minority Over-sampling Technique (SMOTE) produce a more generalized decision region for the minority class compared to simple random oversampling?",
    expectedItems: [],
    expectedAnswer:
      "Instead of simply duplicating existing minority class examples, SMOTE generates brand new synthetic examples by taking the feature vector of a minority instance, finding the difference between it and its nearest neighbor, multiplying that difference by a random number between 0 and 1, and adding it back to the original vector. This mechanism creates new data points that randomly lie along the line segments connecting existing minority samples. By filling in the spaces between existing points rather than just duplicating them, SMOTE effectively forces the model to learn a broader and more general decision region for the minority class.",
    expectedBehavior:
      "Must describe the mathematical mechanism of SMOTE (creating points along the line segment between a sample and its nearest neighbor). Must explain that generating these interpolated synthetic points forces the decision boundary to generalize, rather than just overfitting to duplicated points.",
    notebookId: "jd72h9qsq5zap11ede5k8rqkx585djmc",
