...
export const mlComparisonFixtures: EvalFixture[] = [
  ...
  },
  {
    ...
    expectedItems: [],
    expectedAnswer: "Both `GridSearchCV` and `RandomizedSearchCV` are automated hyperparameter optimization tools provided by scikit-learn. `GridSearchCV` performs an exhaustive search by considering the product of the hyperparameter sets and evaluating each combination one by one. In contrast, `RandomizedSearchCV` samples hyperparameter configurations at random until a specified budget (defined by `n_iter`) is exhausted. `RandomizedSearchCV` is generally faster than `GridSearchCV`, especially when some parameters are more important than others, because evaluating parameters that do not influence performance does not negatively impact efficiency. Additionally, `RandomizedSearchCV` allows users to pass probability distributions to draw from, whereas `GridSearchCV` requires a predefined list of discrete values.",
    expectedBehavior: "Must identify both as hyperparameter optimization techniques. Must contrast the exhaustive search method of `GridSearchCV` with the random sampling and budget-limited search of `RandomizedSearchCV`. Must mention that `RandomizedSearchCV` is generally faster and supports passing probability distributions.",
    notebookId: "jd72h9qsq5zap11ede5k8rqkx585djmc",
  ...
  },
  {
    ...
    question: "Compare macro average and weighted average when evaluating multi-class classification models.",
    expectedItems: [],
    expectedAnswer: "Macro average and weighted average are both techniques used to aggregate binary classification metrics (like precision, recall, and F1-score) across multiple classes. The macro average gives equal importance to all classes and computes the simple average of the metrics over all classes. This metric is preferred if you want to ensure each class is considered equally important, regardless of its size. The weighted average, on the other hand, weights the metric of each class by the number or proportion of samples in that class, and divides by the total number of samples. The weighted average should be used if each individual example in the dataset is considered equally important.",
    expectedBehavior: "Must define both as ways to aggregate metrics for multi-class problems. Must explain the calculation differences (simple average of all classes vs. weighted by the proportion of examples per class). Must clearly state when to use each (when classes are equally important vs. when individual examples are equally important).",
    notebookId: "jd72h9qsq5zap11ede5k8rqkx585djmc",
