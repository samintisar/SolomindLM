...
export const mlAmbiguousFixtures: EvalFixture[] = [
  {
    ...
    question:
      'What does the term "error" refer to in the context of the machine learning course material?',
    expectedItems: [],
    expectedAnswer:
      'In machine learning, "error" refers to different concepts depending on the context. When evaluating model generalization and the fundamental tradeoff, it refers to the training error (error on the training data), the validation error (error on a validation split), the test error (error on unseen test data), or the best possible error for a given problem. Alternatively, in the context of evaluating a binary classification model using a confusion matrix, an error can be a Type I error (a false positive, where the model incorrectly flags an example) or a Type II error (a false negative, where the model misses the example).',
    expectedBehavior:
      "Must disambiguate between generalization errors (training, validation, test) and classification error types (Type I/False Positive, Type II/False Negative).",
  ...
  },
  {
    ...
    question:
      'What is a "baseline" in the context of standard supervised machine learning versus recommender systems?',
    expectedItems: [],
    expectedAnswer:
      "In standard supervised machine learning, a baseline is a simple algorithm based on simple rules of thumb used to sanity-check a model, such as a `DummyClassifier` that always predicts the most frequent label, or a `DummyRegressor` that predicts the mean, median, or a constant value of the training set. In the context of recommender systems, baselines are approaches used to fill in missing entries in a utility matrix, which include using the global average rating, the per-user average rating, or the per-item average rating.",
    expectedBehavior:
      'Must disambiguate the term "baseline" by explaining the `DummyClassifier`/`DummyRegressor` approaches for supervised learning and the average-based imputation approaches (global, user, item) for recommender systems.',
    notebookId: "jd72h9qsq5zap11ede5k8rqkx585djmc",
    tags: ["ambiguous", "ml"],
    scenarioCategory: "ambiguous",
    runner: "chat",
  },
  {
    ...
    question:
      'How does the definition of a "target" differ between standard classification/regression tasks and survival analysis?',
    expectedItems: [],
    expectedAnswer:
      'In standard classification and regression tasks, the target is typically a single feature or column that the model is trying to predict, such as a continuous value for housing prices or a discrete class for disease prediction. In survival analysis, the target is more complex due to right censoring, requiring both the "time to event" (such as customer tenure or subscription length) and information on whether the event actually occurred or was censored (such as whether the customer churned).',
    expectedBehavior:
      "Must contrast the standard single-feature target (continuous or discrete) with the survival analysis target, which requires both the time duration and the censorship/event status.",
    notebookId: "jd72h9qsq5zap11ede5k8rqkx585djmc",
    tags: ["ambiguous", "ml"],
    scenarioCategory: "ambiguous",
    runner: "chat",
  },
  {
    ...
    question:
      'What does the term "weight" refer to in the context of linear models compared to the context of class imbalance?',
    expectedItems: [],
    expectedAnswer:
    ...
    expectedBehavior:
      "Must distinguish between weights as learned parameters (coefficients) in linear models and weights as a hyperparameter (`class_weight`) used to adjust the importance of classes during training.",
    notebookId: "jd72h9qsq5zap11ede5k8rqkx585djmc",
    tags: ["ambiguous", "ml"],
    scenarioCategory: "ambiguous",
    runner: "chat",
