...
interface SlideCandidateOutputInvoker {
  invoke(
    ...
    config?: any
  ): Promise<SlideCandidateResponse>;
}
...
async function generateSlideImage(
  ...
  console.log(
    `[SlideDeckJob] Slide ${slideNumber} generated in ${elapsed}ms (${(imageData.length / 1024).toFixed(2)} KB)`
  );
...
export async function runSlideDeckGenerationPhase(
  ...
  try {
    ...
    console.log(
      `[SlideDeckJob] Packed ${rawChunks.length} chunks into ${packedChunks.length} map tasks`
    );
...
export async function runProcessSlideDeckMapChunkPhase(
  ...
  try {
    ...
    const slideDeck = await ctx.runQuery(internal.studio.slides.index.getInternal, {
      id: slideDeckId,
    });
    if (!slideDeck) {
    ...
    const updatedSlideDeck = await ctx.runQuery(internal.studio.slides.index.getInternal, {
      id: slideDeckId,
    });
    ...
    const failedMaps = slideDeck.metadata?.mapResults
      ? Object.values(slideDeck.metadata.mapResults).filter((r: any) => {
          try {
...
export async function runFinalizeSlideDeckPhase(
  ...
  try {
    ...
    console.log(
      `[SlideDeckJob] Finalization: ${allCandidates.length} candidates collected, ${failedCount.count} failed chunks`
    );
    ...
    if (dedupedSlides.length <= maxSlides) {
      console.log(
        `[SlideDeckJob] Skipping LLM selection: ${dedupedSlides.length} slides within limit`
      );
      ...
      try {
        ...
        console.log(
          `[SlideDeckJob] LLM selection completed in ${Date.now() - startTime}ms, selected ${selectedCandidates.length} slides`
        );
      } catch (error) {
        console.log(`[SlideDeckJob] LLM selection failed, using heuristic fallback: ${error}`);
        selectedCandidates = selectSlidesHeuristic(
          preSelectedSlides,
          targetSlideCount,
          minSlides,
          maxSlides
    ...
    console.log(
      `[SlideDeckJob] Refining ${selectedCandidates.length} slides with concurrency ${CONFIG.REFINE_CONCURRENCY}`
    );
    ...
    console.log(
      `[SlideDeckJob] Refined ${slidesWithPrompts.length} slides (${refinedResults.length - slidesWithPrompts.length} failed)`
    );
    ...
    for (let i = 0; i < slidesWithPrompts.length; i++) {
      ...
      if (i > 0) {
        console.log(
          `[SlideDeckJob] Waiting ${CONFIG.IMAGE_DELAY_MS}ms before generating slide ${slide.slideNumber}...`
        );
      ...
      try {
        ...
        console.error(
          `[SlideDeckJob] CRITICAL: Failed to generate image for slide ${slide.slideNumber}: ${errorMsg}`
        );
        throw new Error(
          `Failed to generate image for slide ${slide.slideNumber} ("${slide.title}"): ${errorMsg}`,
          {
            cause: error,
          }
        );
    ...
    try {
      const titleContent = finalSlides
        .map((s) => s.title)
        .join(" ")
        .substring(0, 2000);
    ...
    await ctx.runMutation(internal.studio.jobMutations.slides.clearSlideDeckMapData, {
      slideDeckId,
    });
