"use node";
/**
 * SlideDeckGenerationJob - Multi-Phase Architecture
 *
 * Breaks slide deck generation into separate scheduled actions to avoid
...
export const slideDeckGeneration = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.runMutation(internal.studio._helpers.updateSlideDeckStatus, {
        slideDeckId,
        status: 'generating',
        metadata: {
          phase: 'initializing',
      ...
      await ctx.runMutation(internal.studio._helpers.initSlideDeckMapPhase, {
        slideDeckId,
      ...
      await ctx.runMutation(internal.studio._helpers.markSlideDeckFailed, {
        slideDeckId,
...
export const processSlideDeckMapChunk = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.runMutation(internal.studio._helpers.storeSlideDeckMapResult, {
        slideDeckId,
        chunkIndex,
...
export const finalizeSlideDeckPhase = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      // Save results
      await ctx.runMutation(internal.studio._helpers.saveSlideDeckResults, {
        slideDeckId,
        slides: finalSlides,
        metadata: {
          title,
      ...
      await ctx.runMutation(internal.studio._helpers.clearSlideDeckMapData, { slideDeckId });
