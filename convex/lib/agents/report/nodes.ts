...
const GRAPH_CONFIG = {
  ...
  // Use env.ts defaults (600000ms = 10 minutes) - no fallback needed
...
const PROCESSING_CONFIG = {
  ...
  // Per-chunk timeout (3 minutes) to prevent one hanging chunk from blocking everything
  // This is more aggressive than the overall 10-minute action timeout
...
export class ReportGraph {
  ...
  private extractTopicsFromOutput(output: string): string[] {
    ...
    try {
      ...
      // Skip error markers from failed chunks
...
2. "summary": The complete structured summary as described above`;
    ...
    try {
      mapOutput = await this.invokeWithRetry<MapOutput>(
        () => this.invokeWithTimeout(
          ...
          PROCESSING_CONFIG.PER_CHUNK_TIMEOUT_MS, // Use aggressive per-chunk timeout
          `Map ${chunkId}`
        ),
        1, // Only retry once for map phase to avoid wasting time on problematic chunks
        `Map ${chunkId}`
      );
    } catch (error) {
      const elapsed = Date.now() - startTime;
      ...
      // Return an error marker instead of throwing - allows collapse phase to handle partial failures
  ...
  async collapse(state: OverallStateType): Promise<Partial<OverallStateType>> {
    ...
    // Separate valid outputs from error markers
    ...
    state.mapOutputs.forEach((output, idx) => {
      try {
        ...
        if (parsed._error === true) {
          // This is an error marker from a failed chunk
          errorOutputs.push({
            index: idx,
            error: {
    ...
    // Check if we have any valid outputs to work with
    ...
    // Extract summaries from valid MapOutput JSON
    ...
    const { topics: topicDistribution, allTopics } = this.analyzeAllTopics(validOutputs);
