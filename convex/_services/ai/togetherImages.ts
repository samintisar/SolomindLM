...
export async function generateInfographicImage(
  ...
  if (resp.data) {
    ...
    console.log(`[TogetherImage] data[0] keys: ${resp.data[0] ? Object.keys(resp.data[0]).join(", ") : "undefined"}`);
  }

  const imageData = resp.data?.[0];
  if (!imageData) {
    throw new Error(`No image data in response. Response: ${JSON.stringify({ ...resp, data: undefined })}`);
  }
  ...
  throw new Error(`No image URL or base64 data in response. Available keys: ${Object.keys(imageData).join(", ")}`);
}
