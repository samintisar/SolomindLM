...
- Same class of mistake: wrong $$\\text{softmax}($z_i$)=\\frac{e^{z_i}}{\\sum_{j=1}^{K}e^{z_j}}$$ — never wrap $z_i$ in extra dollars inside display math. Right: $$\\text{softmax}(z_i)=\\frac{e^{z_i}}{\\sum_{j=1}^{K}e^{z_j}}$$.
