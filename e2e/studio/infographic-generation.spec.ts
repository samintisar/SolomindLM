...
test.describe("Infographic Generation", () => {
  async function createInfographic(page: import("@playwright/test").Page) {
  ...
  test("creation creates placeholder with generating status", async ({ notebookPage }) => {
    test.skip(
      shouldSkipAITests(),
      "Requires source ingestion + infographic job (E2E_AI_ENABLED=1)"
    );

    const page = notebookPage;
    const { card: noteEl } = await createInfographic(page);
  ...
  test("transitions to completed status", async ({ notebookPage }) => {
    test.skip(shouldSkipAITests(), "Requires AI LLM for infographic generation");
