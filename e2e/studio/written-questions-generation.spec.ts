...
test.describe("Written Questions Generation", () => {
  async function createWrittenQuestions(page: import("@playwright/test").Page) {
    ...
    await page.getByRole("button", { name: "Generate Written Questions" }).click();
  ...
  test("creation creates placeholder with generating status", async ({ notebookPage }) => {
    test.skip(
      shouldSkipAITests(),
      "Requires source ingestion + written questions job (E2E_AI_ENABLED=1)"
    );
  ...
  test("transitions to completed status", async ({ notebookPage }) => {
    test.skip(shouldSkipAITests(), "Requires AI LLM for written questions generation");
