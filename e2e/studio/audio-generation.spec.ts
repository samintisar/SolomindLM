...
test.describe("Audio Overview Generation", () => {
  async function createAudioOverview(page: import("@playwright/test").Page) {
  ...
  test("creation creates placeholder with generating status", async ({ notebookPage }) => {
    test.skip(shouldSkipAITests(), "Requires source ingestion + audio job (E2E_AI_ENABLED=1)");

    const page = notebookPage;
    const { card: noteEl } = await createAudioOverview(page);
  ...
  test("transitions to completed status", async ({ notebookPage }) => {
    test.skip(shouldSkipAITests(), "Requires AI LLM for audio generation");
