...
test.describe("Spreadsheet Generation", () => {
  async function createSpreadsheet(page: import("@playwright/test").Page) {
    ...
    // Click on the "Data Table" format card (first preset after "Create Your Own")
  ...
  test("with no sources shows needs-sources dialog", async ({ notebookPage }) => {
    ...
    // Click on the "Data Table" format card
    await page.getByText("Data Table", { exact: true }).click();
  ...
  test("creation creates placeholder with generating status", async ({ notebookPage }) => {
    test.skip(
      shouldSkipAITests(),
      "Requires source ingestion + spreadsheet job (E2E_AI_ENABLED=1)"
    );
